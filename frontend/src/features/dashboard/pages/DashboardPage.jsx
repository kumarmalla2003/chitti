...
const DashboardPage = () => {
  ...
  const calculateStats = ({ chits, members, collections, payouts }) => {
    ...
    // TARGETS (handle different chit types)
  ...
  const getPendingData = () => {
    ...
    data.members.forEach((member) => {
      ...
      member.assignments.forEach((assignment) => {
        ...
        // Calculate expected amount based on chit type
        ...
        // Auction: expectedAmount stays 0 (manually entered during collection)
        ...
        if (expectedAmount > 0 && paidThisMonth < expectedAmount) {
          rawPendingList.push({
