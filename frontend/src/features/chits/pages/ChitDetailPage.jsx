...
import ChitReportPDF from "../components/ChitReportPDF";
...
import ChitDetailsForm from "../components/ChitDetailsForm";
...
import ChitMembersManager from "../components/ChitMembersManager";
...
import { useChit, useCreateChit, useUpdateChit } from "../hooks/useChit";
import { getPayoutsByChitId } from "../../payouts/api/payoutsService";
import { getAssignmentsForChit } from "../api/assignmentsService";
import { getCollectionsByChitId } from "../../collections/api/collectionsService";
...
// DetailsSectionComponent, PayoutsSectionComponent, MembersSectionComponent, DesktopActionButton, TabButton, MobileContent
// Just ensure they are included or I can use the same code as before but with hooks logic injected.

// For brevity, I'll focus on the main component logic. I'll include the helpers to be safe.
...
const ChitDetailPage = () => {
  ...
  // --- React Query Hooks ---
  const { data: chit, isLoading: pageLoading, error: queryError } = useChit(id);
  const createChitMutation = useCreateChit();
  const updateChitMutation = useUpdateChit();

  const loading = createChitMutation.isPending || updateChitMutation.isPending;
  ...
  // Populate form data when chit data is fetched
  useEffect(() => {
    if (chit && mode !== "create") {
      const fetchedData = {
        name: chit.name,
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === "create" && !createdChitId) {
      createChitMutation.mutate(dataToSend, {
        onSuccess: (newChit) => {
        ...
        // ... (Patch logic similar to edit but for created id) ...
        const patchData = { ...dataToSend }; // simplified for brevity
        updateChitMutation.mutate({ id: createdChitId, data: patchData }, {
            onSuccess: (updatedChit) => {
        ...
        updateChitMutation.mutate({ id, data: dataToSend }, {
            onSuccess: () => {
  ...
  const handlePrintReport = async () => {
    ...
    setError(null);
    try {
      // Note: We're not using tokens here anymore
      ...
      const reportProps = {
        chit: { ...originalData, ...formData, id: id },
  ...
  // ... (Return JSX - mostly same structure) ...
  return (
    <>
      <div
