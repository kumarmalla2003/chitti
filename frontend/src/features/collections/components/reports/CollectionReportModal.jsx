// frontend/src/features/collections/components/reports/CollectionReportModal.jsx
  ...
  ({ name, icon: Icon, label, activeTab, setActiveTab }, ref) => {
    ...
    return (
      <button
        ...
        className={`flex-1 flex-shrink-0 flex items-center justify-center gap-2 px-4 py-3 text-sm font-semibold transition-colors duration-200 focus:outline-none rounded-t-md border-b-2 ${isActive
...
const CollectionReportModal = ({
  ...
  const getInfoMessage = () => {
    if (activeTab === "date") {
      return "Generates a report of all collections within the selected date range.";
    } else if (activeTab === "chit") {
      const name =
        chits.find((c) => c.id.toString() === selectedChit)?.name ||
        "the selected chit";
      return selectedChit
        ? `Generates the complete collection history for "${name}".`
                  ...
                  {["thisMonth", "lastMonth", "thisYear"].map((range) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-4 py-1.5 text-xs font-semibold rounded-full transition-colors border ${isActive
