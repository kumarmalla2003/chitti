...
import { getAllCollections } from "../api/collectionsService"; // Keep for report
...
import CollectionReportPDF from "../components/reports/CollectionReportPDF"; // Assuming not moved or path corrected
...
const CollectionsPage = () => {
  ...
  // --- React Query ---
  const { data, isLoading: loading, error: queryError } = useCollections();
  ...
  const handleGenerateReport = async (filters) => {
    ...
    setError(null);
    try {
      const data = await getAllCollections(filters); // Direct service call for report
        ...
        <div className="pb-16 md:pb-0">
          <main className="flex-grow min-h-[calc(100vh-128px)] bg-background-primary px-4 py-8">
            <div className="container mx-auto">
              ...
              {!loading && !queryError && filteredCollections.length === 0 && (
